(function () {
const { links = [], profile = {}, resume = {}, works = [] } = window.siteContent || {};
...
function mountWorks() {
      ...
      (w, idx) => `
        <article class="work-card reveal" tabindex="0" role="button" aria-label="打开作品：${escapeHtml(w.title)}" data-work-index="${idx}">
          <div class="work-card__media">
            <img alt="${escapeAttr(w.title)}" loading="lazy" decoding="async" data-work-index="${idx}" src="${escapeAttr(buildWorkPhotoUrl(w, idx))}" />
          </div>
  ...
  $all("img[data-work-index]", grid).forEach((img) => {
    const idx = Number(img.getAttribute("data-work-index"));
    const w = works[idx];
    mountImageFallback(img, buildWorkFallbackUrl(w, idx));
  });
...
function renderModal() {
  ...
  if (media) {
    ...
    img.alt = String(w.title || "");
    img.decoding = "async";
    img.src = buildWorkPhotoUrl(w, state.modalIndex, true);
    mountImageFallback(img, buildWorkFallbackUrl(w, state.modalIndex, true));
...
function buildAvatarUrl(seed) {
  const options = [
    "./assets/avatars/boy-01.svg",
    "./assets/avatars/boy-02.svg",
    "./assets/avatars/boy-03.svg",
    "./assets/avatars/boy-04.svg",
    "./assets/avatars/boy-05.svg",
    "./assets/avatars/boy-06.svg"
  ];
  return pickBySeed(seed, options);
}

function buildWorkPhotoUrl(work, index, isModal = false) {
  const override = String(work?.photo || "").trim();
  ...
  const street = [
    "./assets/works/photos/street-01.jpg",
    "./assets/works/photos/street-02.jpg",
    "./assets/works/photos/street-03.jpg"
  ];
  const travel = [
    "./assets/works/photos/travel-01.jpg",
    "./assets/works/photos/travel-02.jpg",
    "./assets/works/photos/travel-03.jpg"
  ];
  const landscape = [
    "./assets/works/photos/landscape-01.jpg",
    "./assets/works/photos/landscape-02.jpg",
    "./assets/works/photos/landscape-03.jpg"
  ];

  const pool =
    type.includes("street") || type.includes("街头") || type.includes("胶片")
      ? street
      : type.includes("travel") || type.includes("旅行") || type.includes("日常")
        ? travel
        : landscape;

  return pickBySeed(`${baseSeed}-${index}-${type}-${isModal ? "m" : "c"}`, pool);
}

function buildWorkFallbackUrl(work, index, isModal = false) {
  const baseSeed = getStableSeed("jp_photo_fallback_seed", profile.name || "fallback");
  const type = String(work?.photoType || work?.tag || "").toLowerCase();

  const street = ["./assets/works/street-01.svg", "./assets/works/street-02.svg", "./assets/works/street-03.svg"];
  const travel = ["./assets/works/travel-01.svg", "./assets/works/travel-02.svg", "./assets/works/travel-03.svg"];
  const landscape = [
    "./assets/works/landscape-01.svg",
    "./assets/works/landscape-02.svg",
    "./assets/works/landscape-03.svg"
  ];
...
function mountImageFallback(img, fallbackSrc) {
  if (!img) return;
